...
export default function SignupPage() {
  ...
  // (the "redirect authenticated users away from /signup" half of useAuth).
  useAuth();
    ...
    <div className="min-h-screen bg-[#060d1f] flex">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[55%] px-16 py-14 relative overflow-hidden">
        ...
        <div className="relative space-y-7 max-w-xl">
          <p className="text-blue-400 text-xs font-semibold tracking-[0.2em] uppercase">Free forever</p>
          ...
          <p className="text-slate-400 text-lg leading-relaxed">
            Create your free account and start learning with AI-powered tools built for students.
          </p>
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            {perks.map((f) => (
              <div
                key={f.title}
                className="flex items-start gap-3 bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.07] rounded-2xl px-4 py-3.5 transition-colors duration-200"
              >
        ...
        <p className="relative text-slate-600 text-xs">
          © {new Date().getFullYear()} AI Tutor — built for curious minds.
        </p>
      </div>
